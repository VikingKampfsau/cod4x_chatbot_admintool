...
void getPlayerIP(scr_entref_t entNum)
{
  // Param check
  if(Plugin_Scr_GetNumParam())
    Plugin_Scr_Error("Usage: <player> getPlayerIP();");
  ...
  char address[128];
  client_t* client = Plugin_GetClientForClientNum(entity->s.clientNum);

  if(client->netchan.remoteAddress.type != NA_IP6)
    Plugin_NET_AdrToStringMT( &client->netchan.remoteAddress, address, sizeof(address) );
  else
  {
    //Use the last 4 octets xD
    sprintf(address, "%u.%u.%u.%u:%u",
    client->netchan.remoteAddress.ip6[12], 
    client->netchan.remoteAddress.ip6[13], 
    client->netchan.remoteAddress.ip6[14], 
    client->netchan.remoteAddress.ip6[15], 
    BigShort(client->netchan.remoteAddress.port));
  }
  
  Plugin_Scr_AddString(address);
}

void removeAddedScriptCommand()
{
  // Param check
